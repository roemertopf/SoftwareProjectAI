...
public class Robo extends UDPServer{
	...
	public String [] playerTypes={"keeper", "striker", "back", "back", "back", "back", "back", "back", "back", "back", "striker"};
	public int playerPosition [][]={ {-40,10},{-20,0},{-20,-24},{-20,-16},{-20,-8},{-20,0},{-20,+8},{-20,+16},{-20,+24},{-20,+32},{-10,0}};
	
	//Spiel-Informationen
	public int goals_me=0;						//Eigene Tore
	...
	public double lastX = 0.0;					// Vorherige X-Position	FIXME
	public double lastY = 0.0;					// Vorherige Y-Position FIXME
	...
	// Variablen für den Torwart /*FIXME*/
	...
	*/
	...
	public void tactic_striker () throws Exception{
		...
		//Wenn ich den Ball nun sehe, schaue ich, ob die Entfernung größer als 0.5 ist.
			...
			if(seeGoal){
				//Ich sehe das Tor.
				if(goalDist>27){
					//Das Tor ist noch weiter weg als 27
					...
					if(goalAngle>1||goalAngle<-1){
						...
						if(seeCornerGoalLeft&&seeCornerGoalRight){	
							double kickAngle= GetKickAngle(cornerGoalLeftAngle+kick_offset, cornerGoalRightAngle-kick_offset,10,100,false);
	...
	public void keeper_blockBall() throws Exception{
				...
				Math.abs(desiredY - lastDesiredY) <= desiredOffset )
		{
			...
			case 4:	// Torwart soll nun von den gewünschten Koordinaten aus (wo er gerade steht) zum Ball schauen
				...
				//FIXME ZUSATZ:	wenn zB eine inkrementiere Laufvariable einen bestimmten Werte erreicht hat,
	...
	public void v0_keeper_blockBall(double desired_X) throws Exception{
		...
		if( ballSpeed < 0 ){//FIXME
			...
			case 2:		// Der Torwart steht nun in Richtung der gewünschten Koordinaten und muss nun solange laufen, bis er da ist
				// FIXME mögliches Problem: wenn der Torwart die eine Koordinate schon erreicht hat, die andere aber nicht, läuft er zu weit!
		...
		if( !ballCatched )	// Wenn der Ball nicht gefangen wurde
		{
			...
			if( seeBall && (ballDist < catchable_area_l * 0.8) && (Math.abs(x) > 35) && (Math.abs(y) < 19) ){
				...
				if(ballX > -20){					// Wenn der Ball in den ersten 20 Metern der eigenen Hälfte ist
					...
					if( ballSpeed < 0.5 && (Math.abs(ballX) > 35) && (Math.abs(ballY) < 19) )
					{
	...
	public void parse_init(String msg){
		...
		if(params[0].equals("l")){
			...
			kick_offset=4;
/*FIXME*/	penaltyCenter = "(f p l c)";
/*FIXME*/	goalCenter = "(g l)";			//eigenes Tor
		}
		else{
			...
			kick_offset=-6;
/*FIXME*/	penaltyCenter = "(f p r c)";
/*FIXME*/	goalCenter = "(g r)";			//eigenes Tor
	...
	public void parse_hear(String msg) throws Exception{ /*FIXME*/
		/*
...
/*FIXME*/	seePenaltyCenter 	= false;
/*FIXME*/	seeGoalCenter	 	= false;
...
/*FIXME*/		//Nach dem Strafraumlinienmittelpunkt suchen
...
/*FIXME*/		//Nach dem eigenen Tormittelpunkt suchen
...
/*FIXME*/
	...
	public void change_player_type (int UNUM, int PLAYER_TYPE) throws Exception{ /*FIXME*/
		send("(change_player_type " + UNUM + " " + PLAYER_TYPE + ")");
		Thread.sleep(100);
	}
	public void catch_ball(double angle) throws Exception{ /*FIXME*/
		send("(catch "+angle+")");
		Thread.sleep(100);
	}
}
